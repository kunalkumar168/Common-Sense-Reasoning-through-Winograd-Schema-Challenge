...
Usage: python3 sentence_transformer_cross_encoder.py
"""
...
folder_prefix = '/content/drive/MyDrive/CS685 Project/'
...
#We use stsb-roberta-large as base model and set num_labels=1, which predicts a continous score between 0 and 1
...
# evaluator = CECorrelationEvaluator.from_input_examples(dev_samples, name='dpr-dev')
...
pdp_data_path =  folder_prefix + 'pdp60.tsv'
pdp_data = pd.read_csv(pdp_data_path, sep='\t')

pdp_results = []
c = 0
for _, row in pdp_data.iterrows():
  row = dict(row)
  cand = row['candidates'].split(',')
  scores = []
...
wandb.log({'pdp60_fixed_test':(c/60)*100 })
print("Fine Tuned Cross-Encoder: " + model_name + " PDP60 Test Set Performance: ", (c/60)*100)


# DPR Train Set Performance
# dpr_train_dataset_path = processed_path + 'dpr_train.csv'
# dpr_train_dataset = pd.read_csv(dpr_train_dataset_path)
dpr_train_results = []
c = 0
for _, row in dpr_data.iterrows():
...
# dpr_dev_dataset_path = processed_path + 'dpr_dev.csv'
# dpr_dev_dataset = pd.read_csv(dpr_dev_dataset_path)
dpr_dev_results = []
c = 0
for _, row in dpr_data.iterrows():
  row = dict(row)
