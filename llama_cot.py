...
def generateOutputsFewShot(model_name, generator, wsc_data, 
    ...
    for shot in range(shots):
        ...
        for _,row in wsc_data.iterrows():
            ...
            prompts = [ str(value).replace('Question => ','').replace("'","").replace('"','').replace("            "," ").replace("Output =>",os.linesep+prompt_thought+os.linesep+"Output :") for value in key_sentence_incontext_eg[:shot] ]
            prompts = [ "The Output of below sentence considering the Thought is : " + os.linesep +prompt[3:]+ os.linesep + os.linesep for prompt in prompts ]
            
            prompt_string = "".join(prompts)
            question_thought = "Thought : Based on the given two options, select one of the two option that best fits the pronoun in the above sentence."
            input_sentence = prompt_string + 'What is the Output of below sentence considering the Thought ?'+ os.linesep + row['left'] + ' OPTIONS: - ' + cand[0] + ' ' + row['right'] +' - ' + cand[1] + ' ' +  row['right']
            input_sentence += os.linesep + question_thought + os.linesep+"Output : "
        ...
        with open('outputs/' +model_name + '_' + str(shot) + '_shot_cot' + '.csv', 'w', newline='') as output_file:
